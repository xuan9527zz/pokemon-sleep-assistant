(function(root,factory){
  ...
  const DROWSY_POWER_LINES=Object.freeze({
    '萌绿之岛':Object.freeze({value:19563553,label:'8只自然遇见参考线'}),
    '天青沙滩':Object.freeze({value:30491710,label:'8只自然遇见参考线'}),
    '灰褐洞窟':Object.freeze({value:43706487,label:'8只自然遇见参考线'}),
    '白花雪原':Object.freeze({value:65693395,label:'8只自然遇见参考线'}),
    '宝蓝湖畔':Object.freeze({value:68664246,label:'8只自然遇见参考线'}),
    '黄金发电厂':Object.freeze({value:97371700,label:'8只自然遇见参考线'}),
    '琥珀溪谷':Object.freeze({value:97385818,label:'8只自然遇见参考线（社区暂定）',provisional:true}),
    '萌绿之岛 EX':Object.freeze({value:97371700,label:'8只自然遇见参考线'}),
    '天青沙滩 EX':Object.freeze({value:97371700,label:'8只自然遇见参考线'})
  });
  ...
  function penaltyLineForIsland(islandName){return DROWSY_POWER_LINES[String(islandName||'')]||null}
  function calculateDrowsyPower(snorlaxStrength,sleepScore,multiplier=1){return Math.round(Math.max(0,Number(snorlaxStrength)||0)*clamp(Number(sleepScore)||0,0,100)*Math.max(0,Number(multiplier)||1))}
  ...
  function calculateSleepPlan(options={}){
    const currentStrength=Math.max(0,Math.round(Number(options.currentStrength)||0)),bedtimeStrength=Math.max(0,Math.round(Number(options.bedtimeStrength)||0)),penaltyLine=Math.max(0,Math.round(Number(options.penaltyLine)||0)),multiplier=Math.max(0,Number(options.multiplier)||1),totalScore=clamp(Math.round(Number(options.totalScore)||100),1,100),fullPower=calculateDrowsyPower(bedtimeStrength,totalScore,multiplier);
    const base={currentStrength,bedtimeStrength,penaltyLine,multiplier,totalScore,fullPower,sessions:[],lineSatisfied:false};
    if(!currentStrength||!bedtimeStrength||!penaltyLine)return {...base,mode:'needs-input',reason:'填写当前与睡前预计的卡比兽能量后，才会生成分段建议。'};
    if(fullPower<=penaltyLine){return {...base,mode:'single',lineSatisfied:true,reason:'完整睡眠仍未超过参考线，优先保留一次完整睡眠。',sessions:[{index:1,score:totalScore,minutes:Math.round(totalScore*5.1),strength:bedtimeStrength,drowsyPower:fullPower,underLine:true}]}}
    const firstCap=Math.floor(penaltyLine/(currentStrength*multiplier)),secondCap=Math.floor(penaltyLine/(bedtimeStrength*multiplier)),lower=Math.max(5,totalScore-secondCap),upper=Math.min(totalScore-5,firstCap),feasible=lower<=upper;
    let firstScore;
    if(feasible){const balanced=Math.round(totalScore*bedtimeStrength/(currentStrength+bedtimeStrength));firstScore=clamp(balanced,lower,upper)}
    else firstScore=clamp(Math.round(totalScore*bedtimeStrength/(currentStrength+bedtimeStrength)),5,totalScore-5);
    const secondScore=totalScore-firstScore,firstPower=calculateDrowsyPower(currentStrength,firstScore,multiplier),secondPower=calculateDrowsyPower(bedtimeStrength,secondScore,multiplier),sessions=[
      {index:1,score:firstScore,minutes:Math.round(firstScore*5.1),strength:currentStrength,drowsyPower:firstPower,underLine:firstPower<=penaltyLine},
      {index:2,score:secondScore,minutes:Math.round(secondScore*5.1),strength:bedtimeStrength,drowsyPower:secondPower,underLine:secondPower<=penaltyLine}
    ];
    return {...base,mode:'split',feasible,lineSatisfied:sessions.every(session=>session.underLine),firstCap,secondCap,sessions,reason:feasible?'完整睡眠会越过参考线；这组分配让两段都尽量停在参考线内。':'当前两段能量下无法让总计100分都落在参考线内；先给出较均衡的两段分配。'};
  }
  ...
  function calculatePlan(options){
    ...
    const huntTargets=buildHuntTargets(options.pokemon,context&&context.island&&context.island.name,{strategy:options.strategy,catalog:options.catalog}),linePreset=penaltyLineForIsland(context&&context.island&&context.island.name),penaltyLine=Math.max(0,Number(options.penaltyLine)||Number(linePreset&&linePreset.value)||0),sleepBonus=sleepMultiplierForTeam(activity,options.sleepTeam,isEventArea(context)),sleepPlan=calculateSleepPlan({currentStrength:options.currentSnorlaxStrength,bedtimeStrength:options.bedtimeSnorlaxStrength,penaltyLine,multiplier:sleepBonus.multiplier});
    return {activity,pot,targetGoodCamp,targetRecipe,targetRecipes,preparationTeam,outputTeam,currentTeam,budget,action,huntTargets,context,linePreset,sleepBonus,sleepPlan,weekKey:weekKey(now)};
  }
  function defaults(ingredients,now){
    return {schemaVersion:4,islandIndex:0,berries:[],recipeType:'咖喱／浓汤',preparationRecipeTypes:['咖喱／浓汤','沙拉'],activityKey:'normal',goodCamp:false,eventGoodCamp:true,travelTicketPlanned:true,basePot:81,mealGoal:15,targetRecipeId:'',currentSnorlaxStrength:0,bedtimeSnorlaxStrength:0,penaltyLines:{},completedMeals:[],weekKey:weekKey(now),inventory:Object.fromEntries(ingredients.map(name=>[name,0]))};
  }
  function normalizeState(value,ingredients,islandCount,now=new Date()){
    ...
    return {...base,...source,schemaVersion:4,recipeType,preparationRecipeTypes:preparationRecipeTypes.length?preparationRecipeTypes:base.preparationRecipeTypes,activityKey,goodCamp:source.schemaVersion>=3?source.goodCamp===true:false,eventGoodCamp:source.eventGoodCamp!==false,travelTicketPlanned:source.travelTicketPlanned!==false,basePot:clamp(Math.round(source.basePot||base.basePot),1,1000),mealGoal:clamp(Math.round(source.mealGoal||base.mealGoal),1,21),targetRecipeId:String(source.targetRecipeId||''),currentSnorlaxStrength:Math.max(0,Math.round(Number(source.currentSnorlaxStrength)||0)),bedtimeSnorlaxStrength:Math.max(0,Math.round(Number(source.bedtimeSnorlaxStrength)||0)),penaltyLines:source.penaltyLines&&typeof source.penaltyLines==='object'?source.penaltyLines:{},islandIndex:clamp(Math.round(source.islandIndex),0,Math.max(0,islandCount-1)),completedMeals:completed,weekKey:currentWeek,inventory:{...base.inventory,...(source.inventory&&typeof source.inventory==='object'?source.inventory:{})}};
  }
  function mount(options={}){
    ...
    function effectivePenaltyLine(){const name=islands[state.islandIndex]&&islands[state.islandIndex].name,preset=penaltyLineForIsland(name),custom=Number(state.penaltyLines&&state.penaltyLines[name]);return custom>0?custom:Number(preset&&preset.value)||0}
    function syncSleepControls(){
      if(!controls.currentStrength)return;const islandName=islands[state.islandIndex]&&islands[state.islandIndex].name,preset=penaltyLineForIsland(islandName),custom=Number(state.penaltyLines&&state.penaltyLines[islandName]);controls.currentStrength.value=String(state.currentSnorlaxStrength||0);controls.bedtimeStrength.value=String(state.bedtimeSnorlaxStrength||0);controls.penaltyLine.value=String(custom>0?custom:Number(preset&&preset.value)||0);controls.penaltyReset.disabled=!(custom>0);controls.penaltyHint.textContent=preset?`${preset.label}：${Number(preset.value).toLocaleString('zh-CN')}${preset.provisional?'。琥珀数据仍有社区样本差异，可按实测覆盖。':''}`:'此岛屿尚无内置参考线，请按自己的研究记录填写。';
    }
    ...
    function sleepPlanSection(report){
      const plan=report.sleepPlan,section=document.createElement('section');section.className=`weekly-section weekly-sleep-plan mode-${plan.mode}`;const head=document.createElement('div');head.className='weekly-section-head';const bonus=report.sleepBonus.active?`${report.sleepBonus.matched}随队 ×${report.sleepBonus.multiplier}`:report.sleepBonus.required.length?`当前队伍未带${report.sleepBonus.required.join('／')}，按 ×1`:'无活动睡意之力倍率';head.innerHTML=`<div><span>SLEEP PLAN</span><h3>惩罚线与分段睡眠规划</h3></div><small>${bonus}</small>`;const explainer=document.createElement('p');explainer.className='weekly-sleep-explainer';explainer.textContent='这里的“惩罚线”不是游戏减益，而是达到 8 只自然遇见后，继续提高单次睡意之力的边际收益参考线。规划目标偏向增加两次研究的遇见数，不代表总睡意之力最大。';section.append(head,explainer);
      if(plan.mode==='needs-input'){const empty=document.createElement('p');empty.className='weekly-empty';empty.textContent=plan.reason;section.append(empty);return section}
      const stats=document.createElement('div');stats.className='weekly-sleep-stats';[['参考线',plan.penaltyLine],['完整睡眠睡意之力',plan.fullPower],['每日总睡眠分数',plan.totalScore]].forEach(([label,value])=>{const item=document.createElement('span');item.innerHTML=`<small>${label}</small><b>${Number(value).toLocaleString('zh-CN')}</b>`;stats.append(item)});const cards=document.createElement('div');cards.className='weekly-sleep-sessions';plan.sessions.forEach(session=>{const card=document.createElement('article');card.className=session.underLine?'under-line':'over-line';card.innerHTML=`<span>${plan.mode==='single'?'完整睡眠':`第 ${session.index} 段`}</span><b>${session.score} 分 · 约 ${Math.floor(session.minutes/60)}小时${String(session.minutes%60).padStart(2,'0')}分</b><small>以卡比兽能量 ${session.strength.toLocaleString('zh-CN')} 估算</small><strong>睡意之力 ${session.drowsyPower.toLocaleString('zh-CN')}</strong><em>${session.underLine?'参考线内':'超过参考线'}</em>`;cards.append(card)});const note=document.createElement('p');note.className='weekly-sleep-note';note.textContent=`${plan.reason} 游戏每日最多记录两次睡眠，总活力回复上限为 100；实际入睡时间还需结合你的作息安排。`;section.append(stats,cards,note);return section;
    }
    ...
    controls.bedtimeStrength?.addEventListener('input',()=>{state.bedtimeSnorlaxStrength=Math.max(0,Math.round(Number(controls.bedtimeStrength.value)||0));queueSleepInputRender()});
    controls.penaltyLine?.addEventListener('change',()=>{const name=islands[state.islandIndex]&&islands[state.islandIndex].name,value=Math.max(0,Math.round(Number(controls.penaltyLine.value)||0));if(name){state.penaltyLines={...state.penaltyLines};value?state.penaltyLines[name]=value:delete state.penaltyLines[name]}persist();render()});
    controls.penaltyReset?.addEventListener('click',()=>{const name=islands[state.islandIndex]&&islands[state.islandIndex].name;if(name){state.penaltyLines={...state.penaltyLines};delete state.penaltyLines[name]}persist();render()});
