(function(root,factory){
  ...
  const api=factory(strategy,catalog,sleepResearch,gameRules);
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root)root.POKEMON_SLEEP_WEEKLY_PLANNER=api;
})(typeof globalThis!=='undefined'?globalThis:this,function(strategy,catalog,sleepResearch,gameRules){
  'use strict';
  ...
  function plannerOptions(goodCamp,activity,eventArea,extra={}){return {...extra,goodCamp,energyProfile:'average',memberModifier:activityMemberModifier(activity,eventArea)}}
  ...
  function simulateActivityWeek(options={}){
    ...
    const basePotFinal=finalPotCapacity(baseCapacity,goodCamp,activity),needsPot=Number(recipe.total)>basePotFinal,autoPot=options.autoPot!==false,potOptions=plannerOptions(goodCamp,activity,eventArea,{...teamSettings,baseCapacity,targetCapacity:Number(recipe.total)||0,eventMultiplier:Number(activity.potCapacityMultiplier)||1}),potPokemon=(options.pokemon||[]).map(mon=>withCarry(mon,carryBonus));
    const potDeployment=needsPot&&autoPot&&options.planner&&typeof options.planner.calculatePotDeployment==='function'?options.planner.calculatePotDeployment(potPokemon,options.production,potOptions):null,potReachable=!needsPot||Boolean(potDeployment&&potDeployment.reachable),potHoursPerMeal=needsPot&&potReachable?Math.max(0,Number(options.useSafePotHours===false?potDeployment.expectedHours:potDeployment.safeHours)||0):0,maxPotMeals=needsPot?potReachable&&potHoursPerMeal>0?Math.floor(totalHours/potHoursPerMeal):0:requestedMeals,plannedMeals=Math.min(requestedMeals,maxPotMeals),potHours=Math.min(totalHours,potHoursPerMeal*plannedMeals),fieldHours=Math.max(0,totalHours-potHours);
    const preparationRates=teamRates(preparationTeam),outputRates=teamRates(outputTeam),potRates=teamRates(potDeployment&&potDeployment.potTeam&&potDeployment.potTeam.result),inventory=options.inventory&&typeof options.inventory==='object'?options.inventory:{},step=.25;
    let best=null;
    for(let prepareHours=0;prepareHours<=fieldHours+1e-9;prepareHours=Math.min(fieldHours,round(prepareHours+step,2))){
      const outputHours=Math.max(0,fieldHours-prepareHours),totals=ingredientTotals(inventory,[preparationRates,outputRates,potRates],[prepareHours,outputHours,potHours]),meals=craftableMeals(recipe,totals,plannedMeals),helperEnergy=preparationRates.energyPerHour*prepareHours+outputRates.energyPerHour*outputHours+potRates.energyPerHour*potHours,candidate={prepareHours,outputHours,totals,meals,helperEnergy};
      if(!best||meals>best.meals||(meals===best.meals&&helperEnergy>best.helperEnergy))best=candidate;
      if(meals===plannedMeals){best=candidate;break}
      if(prepareHours===fieldHours)break;
    }
    best=best||{prepareHours:0,outputHours:fieldHours,totals:{...inventory},meals:0,helperEnergy:0};
    const cookedMeals=Math.min(plannedMeals,best.meals),cookingMultiplier=activityCookingMultiplier(activity),baseMealEnergy=Math.max(0,Math.floor(Number(options.recipeEnergy&&options.recipeEnergy(recipe))||0)),mealEnergy=Math.floor(baseMealEnergy*cookingMultiplier),cookingEnergy=mealEnergy*cookedMeals,helperEnergy=Math.round(best.helperEnergy),startingEnergy=Math.max(0,Math.round(Number(options.currentSnorlaxStrength)||0)),finalEnergy=startingEnergy+helperEnergy+cookingEnergy;
    const ingredientRows=(recipe.ingredients||[]).map(item=>{const stock=Math.max(0,Number(inventory[item.name])||0),produced=Math.max(0,(Number(best.totals[item.name])||0)-stock),consumed=(Number(item.amount)||0)*cookedMeals,requested=(Number(item.amount)||0)*requestedMeals,end=Math.max(0,stock+produced-consumed);return {name:item.name,perMeal:Number(item.amount)||0,stock:round(stock),produced:round(produced),consumed:round(consumed),end:round(end),requested:round(requested),shortage:round(Math.max(0,requested-stock-produced))}}),stages=[];
    if(best.prepareHours>.01)stages.push({kind:'prepare',label:'食材准备队',hours:round(best.prepareHours,2),team:preparationTeam.members,energy:Math.round(preparationRates.energyPerHour*best.prepareHours)});
    if(potHours>.01)stages.push({kind:'pot',label:`爆锅队（每餐前约 ${round(potHoursPerMeal,1)} 小时）`,hours:round(potHours,2),team:potDeployment&&potDeployment.potTeam&&potDeployment.potTeam.team||[],energy:Math.round(potRates.energyPerHour*potHours),repeated:plannedMeals});
    if(best.outputHours>.01)stages.push({kind:'output',label:'岛屿输出队',hours:round(best.outputHours,2),team:outputTeam.members,energy:Math.round(outputRates.energyPerHour*best.outputHours)});
    const warnings=[];
    if(needsPot&&!autoPot)warnings.push(`目标料理需要 ${recipe.total} 格，当前锅只有 ${basePotFinal} 格；未启用爆锅队，因此不能制作。`);
    else if(needsPot&&!potReachable)warnings.push('盒内爆锅队无法在本次推演中补足目标锅容量。');
    else if(plannedMeals<requestedMeals)warnings.push(`爆锅时间最多覆盖 ${plannedMeals}/${requestedMeals} 餐。`);
    if(cookedMeals<plannedMeals)warnings.push(`按当前库存与盒内产量只能完成 ${cookedMeals}/${plannedMeals} 餐。`);
    if(context&&context.expert)warnings.push('EX 每周追加规则与主／副树果差异尚未进入精确能量，只按所选喜爱树果的常规 2 倍计算。');
    return {available:true,activity,eventArea,days,totalHours,requestedMeals,plannedMeals,cookedMeals,targetRecipe:recipe,preparationTeam,outputTeam,potDeployment,needsPot,potReachable,potHoursPerMeal:round(potHoursPerMeal,2),stages,ingredientRows,startingEnergy,helperEnergy,baseMealEnergy,mealEnergy,cookingMultiplier,cookingEnergy,finalEnergy,warnings,teamSettings,unmodeled:['大成功随机波动','额外填锅食材能量','活动特殊树果掉落']};
  }
  ...
  function defaults(ingredients,now){
    return {schemaVersion:8,islandIndex:0,berries:[],recipeType:'咖喱／浓汤',preparationRecipeTypes:['咖喱／浓汤','沙拉'],activityKey:'normal',goodCamp:false,eventGoodCamp:true,travelTicketPlanned:true,basePot:LIMITS.permanentPot,mealGoal:15,targetRecipeId:'',currentSnorlaxStrength:0,sleepType:'balanced',sleepObjective:'combined',sleepTargetId:'',simulationDays:7,simulationAutoPot:true,completedMeals:[],weekKey:weekKey(now),inventory:Object.fromEntries(ingredients.map(name=>[name,0]))};
  }
  ...
  function mount(options={}){
    ...
    function renderRecipeOptions(clearProgress=false){
      ...
      controls.targetRecipe.disabled=false;const automatic=document.createElement('option');automatic.value='';automatic.textContent='自动：结合当前库存与食谱等级推荐';controls.targetRecipe.append(automatic);rows.forEach(recipe=>{const option=document.createElement('option');option.value=String(recipe.id);option.textContent=`${recipe.name}｜${recipe.total} 格${Number(recipe.total)>directPot?'｜需爆锅':''}${typeof options.recipeEnergy==='function'?`｜${Math.round(options.recipeEnergy(recipe)).toLocaleString('zh-CN')} 能量`:''}`;controls.targetRecipe.append(option)});
    ...
    function syncControls(){const current=syncProfile();controls.island.value=String(state.islandIndex);controls.recipeType.value=state.recipeType;controls.activity.value=state.activityKey;controls.camp.checked=Boolean(state.goodCamp);controls.eventCamp.checked=Boolean(state.eventGoodCamp);controls.travelTicket.checked=Boolean(state.travelTicketPlanned);controls.pot.value=String(state.basePot);controls.mealGoal.value=String(state.mealGoal);if(controls.simulationDays)controls.simulationDays.value=String(state.simulationDays);if(controls.simulationAutoPot)controls.simulationAutoPot.checked=Boolean(state.simulationAutoPot);renderPreparationControls(current);renderBerryControls();renderRecipeOptions();renderInventory();syncSleepControls()}
    ...
    function simulationSection(report){
      ...
      if(!simulations.length){const empty=document.createElement('p');empty.className='weekly-empty';empty.textContent='先选择一个已经做过、且当前目标锅可制作的食谱；准备周可勾选多条料理路线分别比较。';section.append(empty);return section}
      ...
      simulations.forEach(sim=>{
        ...
        const stats=document.createElement('div');stats.className='weekly-simulation-stats';[
          ['起始能量',sim.startingEnergy],['队伍纯能量',sim.helperEnergy],['料理能量',sim.cookingEnergy],['每餐能量',sim.mealEnergy]
        ...
        const audit=document.createElement('p');audit.className='weekly-simulation-audit';audit.textContent=`已调用：岛屿加成 ${sim.teamSettings.islandBonusPct}% · 本周喜爱树果 ${sim.teamSettings.favoriteBerries.join('／')||'未指定'} · 食谱 Lv.${profile&&typeof profile.recipeLevel==='function'?profile.recipeLevel(sim.targetRecipe.id):1} · ${sim.needsPot?`爆锅每餐约 ${sim.potHoursPerMeal} 小时`:'当前锅可直接制作'}。`;
    ...
    controls.simulationAutoPot?.addEventListener('change',()=>{state.simulationAutoPot=controls.simulationAutoPot.checked;persist();render()});
