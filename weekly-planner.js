(function(root,factory){
  ...
  const WEEK_MODE_VIEWS=Object.freeze({
    ...
    preparation:Object.freeze({label:'活动前准备周',showRoutes:true,showRecipe:false,showSimulation:false,showSleep:false,showHunt:false}),
    event:Object.freeze({label:'活动周',showRoutes:false,showRecipe:true,showSimulation:true,showSleep:true,showHunt:false})
  });
  ...
  function mount(options={}){
    ...
    function renderWeekModeControls(current=profileState()){
      ...
      if(controls.activityTools)controls.activityTools.hidden=mode==='normal';if(controls.simulationSettings)controls.simulationSettings.hidden=!view.showSimulation;if(controls.sleepSettings)controls.sleepSettings.hidden=!view.showSleep;
      controls.camp.closest('.weekly-check')?.toggleAttribute('hidden',preparing);controls.eventCamp.closest('.weekly-check')?.toggleAttribute('hidden',!preparing);controls.travelTicket.closest('.weekly-check')?.toggleAttribute('hidden',!preparing);controls.pot.closest('.weekly-field')?.toggleAttribute('hidden',mode==='normal');
    }
    ...
    function render(){
      ...
      if(weekMode==='normal')sections.push(actionSection(report),sleepPlanSection(report),huntSection(report));
      if(controls.selectionOdds){controls.selectionOdds.hidden=weekMode!=='normal';sections.push(controls.selectionOdds)}
      ...
      else sections.push(actionSection(report),simulationSection(report),sleepPlanSection(report),teamSection(report),logisticsDetails(report));
