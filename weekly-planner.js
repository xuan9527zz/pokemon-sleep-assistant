(function(root,factory){
  ...
  function simulateActivityWeek(options={}){
    ...
    for(let tastyMeals=0;tastyMeals<=maxTastyMeals;tastyMeals+=1){
      ...
      for(let index=0;index<=steps;index+=1){
        const prepareHours=Math.min(fieldHours,index*step),outputHours=Math.max(0,fieldHours-prepareHours),totals=ingredientTotals(inventory,[preparationRates,outputRates,tastyRates],[prepareHours,outputHours,tastyHours]),meals=craftableMeals(recipe,totals,plannedMeals),boostedMeals=Math.min(tastyMeals,meals),helperEnergy=preparationRates.energyPerHour*prepareHours+outputRates.energyPerHour*outputHours+tastyRates.energyPerHour*tastyHours,cookingEnergy=baseMealEnergy*(boostedMeals*boostedExpectedMultiplier+(meals-boostedMeals)*baselineExpectedMultiplier),candidate={prepareHours,outputHours,tastyHours,tastyMeals,boostedMeals,totals,meals,helperEnergy,cookingEnergy,totalGain:helperEnergy+cookingEnergy};
        if(!best||candidate.meals>best.meals||(candidate.meals===best.meals&&candidate.totalGain>best.totalGain))best=candidate;
      }
    }
    best=best||{prepareHours:0,outputHours:totalHours,tastyHours:0,tastyMeals:0,boostedMeals:0,totals:{...inventory},meals:0,helperEnergy:outputRates.energyPerHour*totalHours,cookingEnergy:0,totalGain:outputRates.energyPerHour*totalHours};
    const cookedMeals=Math.min(plannedMeals,best.meals),cookingEnergy=Math.round(best.cookingEnergy),mealEnergy=cookedMeals?Math.round(cookingEnergy/cookedMeals):0,helperEnergy=Math.round(best.helperEnergy),startingEnergy=Math.max(0,Math.round(Number(options.currentSnorlaxStrength)||0)),finalEnergy=startingEnergy+helperEnergy+cookingEnergy;
    ...
    if(best.tastyHours>.01)stages.push({kind:'tasty',label:`爆锅队／料理成功 S（每餐前约 ${round(tastyHoursPerMeal,1)} 小时）`,hours:round(best.tastyHours,2),team:tastyDeployment&&tastyDeployment.tastyTeam&&tastyDeployment.tastyTeam.team||[],energy:Math.round(tastyRates.energyPerHour*best.tastyHours),repeated:best.tastyMeals});
    ...
    return {available:true,activity,eventArea,days,totalHours,requestedMeals,plannedMeals,cookedMeals,targetRecipe:recipe,preparationTeam,outputTeam,tastyDeployment,needsPot,capacityReady,basePotFinal,tastyTargetPct,tastyHoursPerMeal:round(tastyHoursPerMeal,2),tastyMeals:best.tastyMeals,boostedMeals:best.boostedMeals,sundayMealShare:sundayShare,baselineExpectedMultiplier,boostedExpectedMultiplier,stages,ingredientRows,startingEnergy,helperEnergy,baseMealEnergy,mealEnergy,cookingMultiplier,cookingEnergy,finalEnergy,warnings,teamSettings,unmodeled:['大成功的实际随机结果','额外填锅食材能量','活动特殊树果掉落']};
  }
  ...
  function defaults(ingredients,now){
    return {schemaVersion:9,islandIndex:0,berries:[],recipeType:'咖喱／浓汤',preparationRecipeTypes:['咖喱／浓汤','沙拉'],activityKey:'normal',goodCamp:false,eventGoodCamp:true,travelTicketPlanned:true,basePot:LIMITS.permanentPot,mealGoal:15,targetRecipeId:'',currentSnorlaxStrength:0,sleepType:'balanced',sleepObjective:'combined',sleepTargetId:'',simulationDays:7,simulationAutoTasty:true,simulationTastyTarget:70,completedMeals:[],weekKey:weekKey(now),inventory:Object.fromEntries(ingredients.map(name=>[name,0]))};
  }
