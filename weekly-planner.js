(function(root,factory){
  ...
  function calculatePlan(options){
    ...
    const huntTargets=buildHuntTargets(options.pokemon,context&&context.island&&context.island.name,{strategy:options.strategy,catalog:options.catalog}),sleepBonus=sleepMultiplierForTeam(activity,options.sleepTeam,isEventArea(context)),sleepOptions={area:context&&context.island&&context.island.name,currentStrength:options.currentSnorlaxStrength,bedtimeStrength:options.bedtimeSnorlaxStrength,multiplier:sleepBonus.multiplier,sleepType:options.sleepType,objective:options.sleepObjective,goodCamp:options.goodCamp===true},sleepPlan=calculateSleepPlan(sleepOptions),sleepSlowdown=sleepResearch.slowdownReference(sleepOptions),sleepMarginal=sleepResearch.marginalReturn({...sleepOptions,energy:options.bedtimeSnorlaxStrength});
    return {activity,pot,targetGoodCamp,targetRecipe,targetRecipes,preparationTeam,outputTeam,currentTeam,budget,action,huntTargets,context,sleepBonus,sleepPlan,sleepSlowdown,sleepMarginal,sleepDataset:sleepResearch.datasetInfo(),weekKey:weekKey(now)};
  }
  function defaults(ingredients,now){
    return {schemaVersion:5,islandIndex:0,berries:[],recipeType:'咖喱／浓汤',preparationRecipeTypes:['咖喱／浓汤','沙拉'],activityKey:'normal',goodCamp:false,eventGoodCamp:true,travelTicketPlanned:true,basePot:81,mealGoal:15,targetRecipeId:'',currentSnorlaxStrength:0,bedtimeSnorlaxStrength:0,sleepType:'balanced',sleepObjective:'combined',completedMeals:[],weekKey:weekKey(now),inventory:Object.fromEntries(ingredients.map(name=>[name,0]))};
  }
  ...
  function mount(options={}){
    ...
    function syncSleepControls(){
      if(!controls.currentStrength)return;controls.currentStrength.value=String(state.currentSnorlaxStrength||0);controls.bedtimeStrength.value=String(state.bedtimeSnorlaxStrength||0);if(controls.sleepType)controls.sleepType.value=state.sleepType;if(controls.sleepObjective)controls.sleepObjective.value=state.sleepObjective;
    }
    ...
    function sleepPlanSection(report){
      const plan=report.sleepPlan,section=document.createElement('section');section.className=`weekly-section weekly-sleep-plan mode-${plan.recommendation||'pending'}`;const head=document.createElement('div');head.className='weekly-section-head';const bonus=report.sleepBonus.active?`${report.sleepBonus.matched}随队 ×${report.sleepBonus.multiplier}`:report.sleepBonus.required.length?`当前队伍未带${report.sleepBonus.required.join('／')}，按 ×1`:'无活动睡意之力倍率';head.innerHTML=`<div><span>SLEEP RESEARCH</span><h3>研究收益曲线与分段睡眠</h3></div><small>${bonus}</small>`;const explainer=document.createElement('p');explainer.className='weekly-sleep-explainer';explainer.textContent='这里不再把“8只遇见线”当作惩罚线。系统按当前岛屿、卡比兽能量与睡眠类型估算梦之碎片＋研究EXP曲线，并逐一比较 18＋82 到 99＋1 的合法分段方案。';section.append(head,explainer);
      if(!plan.available){const empty=document.createElement('p');empty.className='weekly-empty';empty.textContent=plan.reason;section.append(empty);return section}
      const format=value=>Math.round(Number(value)||0).toLocaleString('zh-CN'),objectiveLabel=sleepResearch.OBJECTIVES[plan.objective],slowdown=report.sleepSlowdown,marginal=report.sleepMarginal,stats=document.createElement('div');stats.className='weekly-sleep-stats';[
        ['收益减速参考点',slowdown?`${format(slowdown.energy)} 能量`:'曲线内未出现'],
        ['完整睡眠收益',format(plan.single.objectiveValue)],
        ['最佳分段收益',format(plan.bestSplit.objectiveValue)],
        ['分段差值',`${plan.gainPercent>=0?'+':''}${plan.gainPercent.toFixed(1)}%`]
      ].forEach(([label,value])=>{const item=document.createElement('span');item.innerHTML=`<small>${label}</small><b>${value}</b>`;stats.append(item)});
      function optionCard(result,label,recommended){
        const card=document.createElement('article');card.className=`weekly-sleep-option${recommended?' recommended':''}`;const title=document.createElement('div');title.className='weekly-sleep-option-head';title.innerHTML=`<div><span>${recommended?'推荐方案':'对照方案'}</span><b>${label}</b></div><strong>${objectiveLabel} ${format(result.objectiveValue)}</strong>`;const sessions=document.createElement('div');sessions.className='weekly-sleep-sessions';result.sessions.forEach(session=>{const item=document.createElement('article'),hours=Math.floor(session.minutes/60),minutes=String(session.minutes%60).padStart(2,'0'),reduced=session.reducedRewardSpawns>0?` · ${session.reducedRewardSpawns}只奖励减半`:'';item.innerHTML=`<span>${result.mode==='single'?'完整睡眠':`第 ${session.index} 段`}</span><b>${session.score} 分 · 约 ${hours}小时${minutes}分</b><small>卡比兽能量 ${format(session.energy)} · 睡意之力 ${format(session.drowsyPower)}</small><strong>${session.totalSpawns}只研究 · 碎片 ${format(session.shards)} · EXP ${format(session.researchExp)}</strong><em>${session.campExtra?`含露营券＋${session.campExtra}只`:session.naturalSpawns+'只自然遇见'}${reduced}</em>`;sessions.append(item)});card.append(title,sessions);return card;
      }
      const comparison=document.createElement('div');comparison.className='weekly-sleep-comparison';comparison.append(optionCard(plan.single,'一次睡满 100 分',plan.recommendation==='single'),optionCard(plan.bestSplit,`${plan.bestSplit.scores[0]}＋${plan.bestSplit.scores[1]} 分`,plan.recommendation==='split'));
      const note=document.createElement('p');note.className='weekly-sleep-note';const marginalText=marginal?`以睡前能量继续提高约 ${marginal.energyGainPercent.toFixed(1)}%，预计${objectiveLabel}只增加 ${marginal.rewardGainPercent.toFixed(1)}%（边际效率 ${marginal.elasticity.toFixed(2)}）。`:'';note.textContent=`${plan.reason} ${marginalText} 第二次研究计入每日第 11 只起研究EXP与梦之碎片减半；好露营券额外遇见只计第一段。实际分睡仍应服从作息，不建议只为微小差值打断睡眠。`;
      ...
      section.append(stats,comparison,note,source);return section;
    }
