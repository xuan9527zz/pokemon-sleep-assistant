(function(root,factory){
  ...
  function mount(options={}){
    ...
    function renderSleepTargets(report){
      ...
      (report.huntTargets||[]).forEach(target=>{const button=document.createElement('button'),label=document.createElement('span');button.type='button';button.className=String(target.id)===String(report.sleepTarget&&report.sleepTarget.id)?'selected':'';button.setAttribute('aria-pressed',button.className==='selected'?'true':'false');label.textContent=target.name;button.append(pokemonIcon(target.best||{speciesId:target.id,name:target.name},'small'),label);button.addEventListener('click',()=>{state.sleepTargetId=String(target.id);persist();render()});controls.sleepTarget.append(button)});
    }
