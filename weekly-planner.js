(function(root,factory){
  ...
  function simulateActivityWeek(options={}){
    ...
    const stageSpecs=[best.prepareHours>.01?{kind:'prepare',label:'食材准备队',hours:best.prepareHours,members:preparationTeam.members}:null,best.tastyHours>.01?{kind:'tasty',label:`爆锅队／料理成功 S（每餐前约 ${round(tastyHoursPerMeal,1)} 小时）`,hours:best.tastyHours,members:tastyDeployment&&tastyDeployment.tastyTeam&&tastyDeployment.tastyTeam.team||[],repeated:best.tastyMeals}:null,best.outputHours>.01?{kind:'output',label:'岛屿输出队',hours:best.outputHours,members:outputTeam.members}:null].filter(Boolean),stageReports=stageSpecs.map((stage,index)=>{
      const switchCount=index<stageSpecs.length-1?(stage.kind==='tasty'?Math.max(1,stage.repeated||1):1):0,team=(stage.members||[]).map(mon=>withCarry(mon,carryBonus)),report=stage.hours>=.5&&options.planner&&typeof options.planner.calculateTeam==='function'?options.planner.calculateTeam(team,options.production,plannerOptions(goodCamp,activity,eventArea,{...teamSettings,durationHours:stage.hours,teamSwapCount:switchCount})):null;
      return {...stage,report,energy:report?Number(report.energy.totalEnergy)||0:(stage.kind==='prepare'?preparationRates:stage.kind==='tasty'?tastyRates:outputRates).energyPerHour*stage.hours,lostTriggers:report&&report.energy&&report.energy.timeline?report.energy.timeline.totals.lostTriggers:0};
    }),cookedMeals=Math.min(plannedMeals,best.meals),cookingEnergy=Math.round(best.cookingEnergy),mealEnergy=cookedMeals?Math.round(cookingEnergy/cookedMeals):0,helperEnergy=Math.round(stageReports.length?sum(stageReports.map(stage=>stage.energy)):best.helperEnergy),startingEnergy=Math.max(0,Math.round(Number(options.currentSnorlaxStrength)||0)),finalEnergy=startingEnergy+helperEnergy+cookingEnergy;
    ...
    stageReports.forEach(stage=>stages.push({kind:stage.kind,label:stage.label,hours:round(stage.hours,2),team:stage.members,energy:Math.round(stage.energy),repeated:stage.repeated,lostTriggers:round(stage.lostTriggers,2)}));
    const warnings=[];
