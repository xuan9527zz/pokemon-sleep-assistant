(function(root,factory){
  ...
  const WEEK_MODE_VIEWS=Object.freeze({
    normal:Object.freeze({label:'普通周',showRoutes:false,showRecipe:false,showSimulation:false,showSleep:true,showHunt:true}),
    preparation:Object.freeze({label:'活动前准备周',showRoutes:true,showRecipe:false,showSimulation:false,showSleep:false,showHunt:true}),
    event:Object.freeze({label:'活动周',showRoutes:false,showRecipe:true,showSimulation:true,showSleep:true,showHunt:true})
  });
  ...
  function mount(options={}){
    ...
    function renderWeekModeControls(current=profileState()){
      const mode=Object.hasOwn(WEEK_MODE_VIEWS,current&&current.weekMode)?current.weekMode:'normal',view=WEEK_MODE_VIEWS[mode],preparing=mode==='preparation';rootNode.dataset.weekMode=mode;
      if(controls.preparationRoutes){controls.preparationRoutes.hidden=!view.showRoutes;controls.preparationRoutes.querySelectorAll('input[type="checkbox"]').forEach(input=>{input.checked=state.preparationRecipeTypes.includes(input.value)})}
      controls.recipeType.closest('.weekly-field')?.toggleAttribute('hidden',!view.showRecipe);controls.targetRecipe.closest('.weekly-field')?.toggleAttribute('hidden',!view.showRecipe);controls.mealGoal.closest('.weekly-field')?.toggleAttribute('hidden',mode==='normal');
      if(controls.activityTools)controls.activityTools.hidden=false;if(controls.simulationSettings)controls.simulationSettings.hidden=!view.showSimulation;if(controls.sleepSettings)controls.sleepSettings.hidden=!view.showSleep;
      controls.camp.closest('.weekly-check')?.toggleAttribute('hidden',false);controls.eventCamp.closest('.weekly-check')?.toggleAttribute('hidden',!preparing);controls.travelTicket.closest('.weekly-check')?.toggleAttribute('hidden',!preparing);controls.pot.closest('.weekly-field')?.toggleAttribute('hidden',false);
    }
    ...
    function overviewSection(report){
      const section=document.createElement('section');section.className=`weekly-overview mode-${report.weekMode}`;const island=islands[state.islandIndex].name,missing=report.huntTargets.filter(target=>target.status==='missing').length,upgrades=report.huntTargets.filter(target=>target.status==='upgrade').length,teamCount=report.outputTeam.members.length,shortages=report.budget.rows.filter(row=>row.gapNow>.05).length;
      section.dataset.noIngredientIcons='';
      if(report.weekMode==='normal'){
        section.innerHTML=`<div><span>WEEK TARGET｜普通周</span><h3>${island}培养路线</h3><p>围绕本岛喜爱树果安排输出队；睡眠方案和严选目标单独跟进，不把活动备料混进当前行动。</p></div><div class="weekly-overview-stats"><span><small>岛屿输出队</small><b>${teamCount} / 5</b></span><span><small>缺失岗位</small><b>${missing} 个</b></span><span><small>待升级目标</small><b>${upgrades} 个</b></span><span><small>喜爱树果</small><b>${(report.context&&report.context.berries||[]).join('／')||'未指定'}</b></span></div>`;return section;
      }
      const routes=report.targetRecipes&&report.targetRecipes.length>1?`<p>备料路线：${report.targetRecipes.map(recipe=>`${recipe.type}「${recipe.name}」`).join('；')}。活动周确定料理后只消耗命中路线。</p>`:'',activityNote=report.weekMode==='preparation'?`为「${report.simulationActivity.label}」预先储备`:(Array.isArray(report.activity.eventAreas)&&report.activity.eventAreas.length&&!report.preparationTeam.eventArea?`${report.activity.note} 当前岛屿不在活动区域，因此不套用活动成员加成。`:report.activity.note),title=report.weekMode==='preparation'?`储备 ${shortages} 种目标食材`:`${report.targetRecipe?report.targetRecipe.name:'尚无可用目标食谱'} × ${report.budget.goal}`;
      section.innerHTML=`<div><span>WEEK TARGET｜${WEEK_MODE_VIEWS[report.weekMode].label}</span><h3>${title}</h3><p>${island}｜${activityNote}</p>${routes}</div><div class="weekly-overview-stats"><span><small>${report.weekMode==='preparation'?'储备完成':'目标进度'}</small><b>${report.weekMode==='preparation'?`${report.budget.rows.length-shortages} / ${report.budget.rows.length}`:`${report.budget.completed} / ${report.budget.goal}`}</b></span><span><small>本批规划</small><b>${report.budget.batchMeals} 餐</b></span><span><small>食材缺口</small><b>${shortages} 种</b></span><span><small>仓库占用</small><b>${Math.ceil(report.budget.inventoryTotal)} / ${report.budget.inventoryLimit}</b></span></div>`;return section;
    }
    ...
    function simulationSection(report){
      ...
      simulations.forEach(sim=>{
        ...
        const ingredients=document.createElement('div');ingredients.className='weekly-simulation-ingredients';sim.ingredientRows.forEach(row=>{const item=document.createElement('span');const name=document.createElement('b');name.append(icon(row.name));const values=document.createElement('small');values.textContent=`库存 ${row.stock} ＋ 产出 ${row.produced} － 料理 ${row.consumed} ＝ 剩余 ${row.end}${row.shortage>0?` · 总目标仍缺 ${row.shortage}`:''}`;item.className=row.shortage>0?'short':'';item.append(name,values);ingredients.append(item)});
    ...
    function actionSection(report){
      const normal=report.weekMode==='normal',members=report.outputTeam.members||[],targets=report.huntTargets.filter(target=>target.status!=='covered'),collectionHours=normal?Number(report.outputTeam.production&&report.outputTeam.production.collectionHours)||4:report.action.collectionHours,section=document.createElement('section');section.className=`weekly-action phase-${normal?'output':report.action.phase}`;const top=document.createElement('div');top.className='weekly-action-head';top.innerHTML=`<div><span>NOW｜当前行动</span><h3>${normal?`先跑 ${islands[state.islandIndex].name}适配队`:report.action.title}</h3></div><b>${teamPlanner.formatHours(collectionHours)} 收菜</b>`;const detail=document.createElement('p');detail.textContent=normal?`当前路线先培养并实战检验 ${members.map(mon=>mon.nickname||mon.name).join('、')||'岛屿推荐成员'}。${targets.length?`研究时优先补 ${targets.slice(0,3).map(target=>target.name).join('、')}，新个体达到最低线后再替换。`:'现有严选岗位均已覆盖，本周以等级、岛屿加成和稳定输出为主。'}`:report.action.detail;const thresholds=document.createElement('div');thresholds.className='weekly-action-thresholds';const chips=normal?members.map((mon,index)=>`${index+1}. ${mon.nickname||mon.name} · ${mon.specialtyLabel||'当前定位'}`):report.action.thresholds;chips.forEach(text=>{const chip=document.createElement('span');chip.textContent=text;thresholds.append(chip)});const timing=document.createElement('small');timing.textContent=normal?'下方睡眠优化与严选目标会沿用同一岛屿设置。':`建议下一次收菜：约 ${report.action.collectAt}`;section.append(top,detail);if(chips.length)section.append(thresholds);section.append(timing);return section;
    }
    ...
    function teamSection(report){
      const normal=report.weekMode==='normal',section=document.createElement('section');section.className='weekly-section';section.innerHTML=normal?'<div class="weekly-section-head"><div><span>TRAINING ROUTE</span><h3>本岛培养与输出队</h3></div><small>与当前行动使用同一套岛屿推荐结果</small></div>':'<div class="weekly-section-head"><div><span>TEAM SWITCH</span><h3>现在用队与补齐后队伍</h3></div><small>准备队按目标食材缺口选择；输出队沿用岛屿推荐页</small></div>';const pair=document.createElement('div');pair.className='weekly-team-pair';
      const current=document.createElement('article');current.className='weekly-team-panel';const currentOutput=normal||['output','complete'].includes(report.action.phase);current.innerHTML=`<div class="weekly-team-panel-head"><span>${normal?'普通周主路线':currentOutput?'现在使用':'食材准备阶段'}</span><b>${currentOutput?'岛屿输出队':'目标食材准备队'}</b></div>`;current.append(teamCards(normal?report.outputTeam:report.currentTeam));pair.append(current);
      if(!normal&&!['output','complete'].includes(report.action.phase)){const after=document.createElement('article');after.className='weekly-team-panel';after.innerHTML='<div class="weekly-team-panel-head"><span>食材补齐后</span><b>切换岛屿输出队</b></div>';after.append(teamCards(report.outputTeam));pair.append(after)}else current.classList.add('wide');
    ...
    function budgetSection(report,{preparationTarget=false}={}){
      const section=document.createElement('section');section.className=`weekly-section${preparationTarget?' weekly-stock-target':''}`;section.innerHTML=preparationTarget?`<div class="weekly-section-head"><div><span>STOCK TARGET</span><h3>本周要储存的食材</h3></div><small>库存每次更新后，缺口、预计补齐时间和当前行动都会重新计算</small></div>`:`<div class="weekly-section-head"><div><span>INGREDIENT BUDGET</span><h3>仓库可容纳的下一批食材</h3></div><small>全库存硬上限 ${report.budget.inventoryLimit}；本批最多规划 ${report.budget.batchMeals} 餐，用掉后再滚动计算</small></div>`;const grid=document.createElement('div');grid.className='weekly-budget';
      ...
      report.budget.rows.forEach(row=>{const card=document.createElement('article');card.className=`weekly-budget-row ${row.projectedGap>.05?'short':row.gapNow>.05?'working':'ready'}`;const identity=document.createElement('div');identity.className='weekly-budget-name';const copy=document.createElement('div'),perMeal=document.createElement('small');copy.append(icon(row.name));perMeal.textContent=`每餐 ${row.perMeal}`;copy.append(perMeal);identity.append(copy);const stats=document.createElement('div');stats.className='weekly-budget-stats';[['剩余需求',row.need],['当前库存',row.stock],['准备队／日',row.perDay],['周末预计',row.projected]].forEach(([label,value])=>{const item=document.createElement('span');item.innerHTML=`<small>${label}</small><b>${value}</b>`;stats.append(item)});const status=document.createElement('strong');status.className='weekly-budget-state';status.textContent=row.projectedGap>.05?`预计仍缺 ${Math.ceil(row.projectedGap)}`:row.gapNow>.05?`约 ${row.daysToStock} 天补齐`:'库存已够';card.append(identity,stats,status);grid.append(card)});section.append(grid);return section;
    }
