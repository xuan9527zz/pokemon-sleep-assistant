...
const cookingSuccess = require('../cooking-success.js');
...
const shifts=weekly.buildShiftSchedule(monday,24,4,1,2);
assert.equal(shifts.stepMinutes,15);
assert.ok(shifts.rows.length>=1);
assert.equal(shifts.segments.some(segment=>segment.kind==='prepare'),true);
...
assert.equal(normalized.skillCollectionHours,4);
assert.equal(normalized.collectBeforeSwitch,true);
assert.equal(normalized.exWeeklyEffect,'none');
assert.equal(Object.hasOwn(normalized,'simulationAutoPot'),false);
assert.equal(weekly.normalizeState({simulationTastyTarget:0},['特选苹果'],9,monday).simulationTastyTarget,0,'0%目标应能明确关闭爆锅蓄力');
...
assert.equal(weekly.WEEK_MODE_VIEWS.event.showSimulation,true);
assert.equal(weekly.WEEK_MODE_VIEWS.normal.showHunt,true);
assert.equal(weekly.WEEK_MODE_VIEWS.preparation.showHunt,true);
...
const weeklySource = fs.readFileSync(path.join(projectRoot,'weekly-planner.js'),'utf8');
assert.ok(html.includes('id="weeklyCurrentStrength"'));
assert.ok(html.includes('id="weeklySleepTarget"'));
assert.ok(html.includes('id="weeklySimulationDays"'));
assert.ok(html.includes('id="weeklySimulationAutoTasty"'));
assert.ok(html.includes('id="weeklySimulationTastyTarget"'));
assert.ok(html.includes('id="weeklySkillCollectionHours"'));
assert.ok(html.includes('id="weeklyCollectBeforeSwitch"'));
assert.ok(html.includes('id="weeklyExEffect"'));
assert.ok(html.includes('活动周完整推演器'));
...
assert.equal(realPlan.weekMode,'normal');
assert.equal(realPlan.simulations.length,0,'普通周不应提前生成活动周推演');
const realEventPlan=weekly.calculatePlan({...common,targetRecipeId:target.id,weekMode:'event'});
assert.equal(realEventPlan.simulations.length,1);
assert.equal(realEventPlan.simulations[0].teamSettings.favoriteBerries.join('／'),islandContext.berries.join('／'),'活动周推演必须把本周实际喜爱树果传给队伍能量引擎');
const cookingSimulation=weekly.simulateActivityWeek({...common,activity:weekly.ACTIVITY_PROFILES.cooking125,targetRecipe:recipes[0],days:1,mealGoal:1,inventory:{'特选苹果':20},recipeEnergy:()=>1000,islandBonusPct:50,islandProfile:'lapis',autoTasty:false,currentSnorlaxStrength:12345});
assert.equal(cookingSimulation.available,true);
assert.equal(cookingSimulation.cookedMeals,1);
assert.equal(cookingSimulation.mealEnergy,1375,'活动料理倍率与工作日自然10%大成功率必须共同进入每餐期望能量');
assert.equal(cookingSimulation.cookingEnergy,1375);
assert.equal(cookingSimulation.startingEnergy,12345);
assert.equal(cookingSimulation.teamSettings.islandBonusPct,50);
assert.ok(cookingSimulation.stages.reduce((sum,stage)=>sum+stage.hours,0)<=24.01,'准备、爆锅与输出队必须共享同一推演时长，不能重复计算时间');
assert.ok(cookingSimulation.stages.every(stage=>stage.scheduleText),'每个阶段都应给出可执行的日期时间说明');
assert.ok(cookingSimulation.prepareHoursWithoutStock>=cookingSimulation.stockSavedPrepareHours,'库存节省时长必须来自零库存反事实对照');
...
const directEvent=teamPlanner.calculateTeam(eventTeam,common.production,{goodCamp:true,energyProfile:'timeline',memberModifier:weekly.activityMemberModifier(weekly.ACTIVITY_PROFILES.mewtwo1,true),durationHours:eventStage.hours,...eventSettings});
assert.deepEqual(eventStage.breakdown,weekly.teamBreakdown(directEvent),'活动持有、技能等级／概率、食材帮忙与喜爱树果也必须和当前队伍共享口径');
const fakeTastyPlanner={...teamPlanner,calculateTastyDeployment(){return {reachable:true,estimate:{safeHours:1,state:cookingSuccess.initialBonusState(70)},tastyTeam:{team:[box[0]],result:{energy:{durationHours:24,totalEnergy:0},ingredients:[]}}}}};
const tastySimulation=weekly.simulateActivityWeek({...common,planner:fakeTastyPlanner,activity:weekly.ACTIVITY_PROFILES.normal,targetRecipe:recipes[0],days:1,mealGoal:1,inventory:{'特选苹果':20},recipeEnergy:()=>100000,autoTasty:true,tastyTargetPct:70});
assert.equal(tastySimulation.tastyMeals,1,'高能量料理应选择料理成功 S 爆锅队');
assert.equal(tastySimulation.stages.some(stage=>stage.kind==='tasty'),true);
assert.ok(tastySimulation.cookingEnergy>110000,'爆锅餐应高于只计自然大成功率的料理期望');
const oversizedRecipe={id:99,name:'扩锅测试料理',type:'沙拉',energy:5000,total:100,ingredients:[{name:'特选苹果',amount:100}]};
const oversizedPlan=weekly.calculatePlan({...common,recipes:[...recipes,oversizedRecipe],targetRecipeId:99,basePot:81,goodCamp:false,simulationAutoTasty:false,weekMode:'event'});
assert.equal(oversizedPlan.targetRecipe.id,99,'手动指定的大锅食谱不能在进入扩锅判断前被当前锅容量过滤掉');
assert.equal(oversizedPlan.simulations[0].needsPot,true);
assert.equal(oversizedPlan.simulations[0].cookedMeals,0);
assert.ok(oversizedPlan.simulations[0].warnings.some(text=>text.includes('爆锅队')&&text.includes('不会增加容量')));
...
const preparationWithoutCurrentCamp=weekly.calculatePlan({...common,activityKey:'normal',targetRecipeId:target.id,goodCamp:false,eventGoodCamp:true,weekMode:'preparation'});
const preparationWithCurrentCamp=weekly.calculatePlan({...common,activityKey:'normal',targetRecipeId:target.id,goodCamp:true,eventGoodCamp:true,weekMode:'preparation'});
const totalPreparationIngredients=plan=>Object.values(plan.preparationTeam.daily).reduce((sum,value)=>sum+Number(value||0),0);
assert.equal(preparationWithoutCurrentCamp.pot,preparationWithCurrentCamp.pot,'准备周的未来目标锅只应读取活动周计划好露营券');
assert.ok(totalPreparationIngredients(preparationWithCurrentCamp)>totalPreparationIngredients(preparationWithoutCurrentCamp),'准备周的当前备料产出应读取本周好露营券');
...
assert.equal(dualRoutePlan.simulations.length,0,'准备周只负责储备目标，不应提前显示或生成活动周推演');
