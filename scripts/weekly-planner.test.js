...
assert.equal(weekly.penaltyLineForIsland('宝蓝湖畔').value,68664246);
...
const singleSleep=weekly.calculateSleepPlan({currentStrength:100000,bedtimeStrength:150000,penaltyLine:19563553});
assert.equal(singleSleep.mode,'single');
assert.equal(singleSleep.sessions[0].score,100);
const splitSleep=weekly.calculateSleepPlan({currentStrength:200000,bedtimeStrength:300000,penaltyLine:19563553});
assert.equal(splitSleep.mode,'split');
assert.equal(splitSleep.sessions.reduce((sum,session)=>sum+session.score,0),100);
assert.ok(splitSleep.sessions.every(session=>session.underLine),'可行时两段都应停在8只自然遇见参考线内');
const forcedSplit=weekly.calculateSleepPlan({currentStrength:1000000,bedtimeStrength:1000000,penaltyLine:19563553});
assert.equal(forcedSplit.feasible,false);
assert.equal(forcedSplit.sessions.reduce((sum,session)=>sum+session.score,0),100);
