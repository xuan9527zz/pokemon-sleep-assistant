...
assert.equal(realPlan.simulations.length,1);
assert.equal(realPlan.simulations[0].teamSettings.favoriteBerries.join('／'),islandContext.berries.join('／'),'活动周推演必须把本周实际喜爱树果传给队伍能量引擎');
...
const oversizedPlan=weekly.calculatePlan({...common,recipes:[...recipes,oversizedRecipe],targetRecipeId:99,basePot:81,goodCamp:false,simulationAutoTasty:false});
...
assert.equal(dualRoutePlan.simulations.length,2,'准备周双路线应分别生成完整活动周推演，不能把两种料理混成一条伪食谱');
