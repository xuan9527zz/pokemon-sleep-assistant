...
assert.equal(normalized.simulationAutoPot,true);
...
assert.ok(html.includes('id="weeklySimulationAutoPot"'));
...
const cookingSimulation=weekly.simulateActivityWeek({...common,activity:weekly.ACTIVITY_PROFILES.cooking125,targetRecipe:recipes[0],days:1,mealGoal:1,inventory:{'特选苹果':20},recipeEnergy:()=>1000,islandBonusPct:50,islandProfile:'lapis',autoPot:true,currentSnorlaxStrength:12345});
assert.equal(cookingSimulation.available,true);
assert.equal(cookingSimulation.cookedMeals,1);
assert.equal(cookingSimulation.mealEnergy,1250,'活动料理倍率必须进入每餐能量');
assert.equal(cookingSimulation.cookingEnergy,1250);
...
const oversizedRecipe={id:99,name:'爆锅测试料理',type:'沙拉',energy:5000,total:100,ingredients:[{name:'特选苹果',amount:100}]};
const oversizedPlan=weekly.calculatePlan({...common,recipes:[...recipes,oversizedRecipe],targetRecipeId:99,basePot:81,goodCamp:false,simulationAutoPot:false});
assert.equal(oversizedPlan.targetRecipe.id,99,'手动指定的大锅食谱不能在进入爆锅推演前被当前锅容量过滤掉');
assert.equal(oversizedPlan.simulations[0].needsPot,true);
assert.ok(oversizedPlan.simulations[0].warnings.some(text=>text.includes('未启用爆锅队')));
