(function(root,factory){
  ...
  function mount(options={}){
    ...
    function resetForm(){
      editingId=null;releaseArmed=false;title.textContent='快速录入新个体';releaseButton.hidden=true;duplicateButton.hidden=true;releaseButton.textContent='放生到回收站';speciesSearch.value='';renderSpeciesOptions();if(nicknameInput)nicknameInput.value='';if(customNumberInput)customNumberInput.value='';levelInput.value='1';shinyInput.checked=false;natureSelect.value='认真';prioritySelect.value='按需求保留';boxSelect.value='pending';battleInput.checked=true;noteInput.value='';subskillSelects.forEach(select=>{select.value='—'});updateSpeciesFields();message.hidden=true;
    }
