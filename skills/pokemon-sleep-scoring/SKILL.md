...
7. Treat [scripts/scoring-core.js](scripts/scoring-core.js) as the executable source of truth for individual scoring. Use [scripts/box-scores.js](scripts/box-scores.js) for box-wide snapshots and legal-ceiling audits.
8. Treat [scripts/species-tiers.js](scripts/species-tiers.js) as the executable source of truth for the user-maintained S/A/B/C species tiers. Unlisted final forms default to C; unevolved helpers inherit the selected final form.
9. Treat [scripts/species-scores.js](scripts/species-scores.js) and the team-ranking scripts as internal production diagnostics for team, island, and event calculations. Do not expose or combine them as a user-facing species score.

## Non-negotiable rules

- Display `species tier + individual quality + graduation status`; never combine species performance and an individual panel into one percentage.
- Separate production mechanics, species tier, individual quality, strategic role, cultivation advice, and release advice. Each answers a different question.
...
- Ingredient-route coefficients affect only ingredient-specialist quality or the ingredient channel of an all-rounder; never discount an all-rounder's berry or skill channel and never change the species tier.
- Map an unevolved helper to its selected final form. Score Mew and Darkrai with separate berry/ingredient/skill individual channels. Their fixed Natures do not create an impossible reroll-quality gap, unopened Eureka Seed slots are excluded from the current denominator, and the displayed individual quality uses the user-selected focus or the strongest currently revealed channel. Mew is S tier only with Berry Burst and B with every other All-Mighty skill; Darkrai is S tier.
...
- A strategic-role label may explain a verified team gap, but may not silently change the user-maintained tier, erase a poor individual, or invent output.
...
When a rule changes, update the shared script and its matching reference in the same change. Rebuild the box snapshot (and the catalog only when production data changes), run all tests, and validate both the project copy and installed copy of this skill.
