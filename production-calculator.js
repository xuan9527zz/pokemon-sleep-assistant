(function(root,factory){
  ...
  function comparisonTeam(candidate,baseline,candidateIds){
    const blocked=new Set((candidateIds||[]).map(String)),rest=distinctById(baseline).filter(mon=>String(mon.id)!==String(candidate.id)&&!blocked.has(String(mon.id))&&!SPECIAL_NAMES.has(String(mon.name||''))).slice(0,4);
  ...
  function energyRange(row){
    ...
    return {mean:berryMean+skillMean,...range(berryMean+skillMean,variance)};
  }
  function calculate(mon,targetIngredient,options={}){
    ...
    if(!result||!result.valid)return {valid:false,id:String(mon.id||''),name:String(mon.name||'未知'),message:result&&result.message||'当前生产模型不可用。'};
  ...
  return Object.freeze({Z80,range,comparisonTeam,ingredientRange,energyRange,calculate,compareMany,mewSkillScenarios,formatRange});
});
