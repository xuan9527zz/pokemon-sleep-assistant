(function(root,factory){
  ...
  function compareSleepPlans(options={}){
    const area=areaFor(options.area),currentStrength=Math.max(0,Math.round(Number(options.currentStrength)||0)),bedtimeStrength=Math.max(0,Math.round(Number(options.bedtimeStrength)||0)),sleepType=Object.hasOwn(SLEEP_TYPES,options.sleepType)?options.sleepType:'balanced',objective=Object.hasOwn(OBJECTIVES,options.objective)?options.objective:'combined',multiplier=Math.max(0,Number(options.multiplier)||1),goodCamp=options.goodCamp===true;
    if(!area||!currentStrength||!bedtimeStrength)return {available:false,reason:!area?'当前岛屿暂无研究收益曲线。':'填写第一段入睡时与睡前预计的卡比兽能量后，才会比较完整睡眠和分段睡眠。',area,currentStrength,bedtimeStrength,sleepType,objective,multiplier,goodCamp};
